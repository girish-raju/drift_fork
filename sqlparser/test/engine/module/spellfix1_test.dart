...
final _spellfixOptions = EngineOptions(
  enabledExtensions: const [Spellfix1Extension()],
);
  ...
  group('engine analyze spellfix1', () {
    ...
    test('ignore hidden columns in result', () {
      final result = engine.analyze(
        'SELECT * FROM demo WHERE word MATCH \'none\'',
      );
    ...
    test('accepts hidden columns in query', () {
      final result = engine.analyze(
        'SELECT * FROM demo WHERE word MATCH \'none\' AND top=3',
      );
      expect(result.errors, isEmpty);
    });
