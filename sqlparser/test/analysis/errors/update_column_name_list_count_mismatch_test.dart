...
void main() {
  ...
  group("using column-name-list and values in update", () {
    test('reports error if they have different sizes', () {
      engine
          .analyze("UPDATE foo SET (a, b) = (VALUES(1));")
          .expectError(
            "(VALUES(1))",
            type: AnalysisErrorType.cteColumnCountMismatch,
          );
      engine
          .analyze("UPDATE foo SET (a) = (VALUES(1,2));")
          .expectError(
            "(VALUES(1,2))",
            type: AnalysisErrorType.cteColumnCountMismatch,
          );
  ...
  group("using column-name-list and subquery in update", () {
    test('reports error if they have different sizes', () {
      engine
          .analyze("UPDATE foo SET (a, b) = (SELECT 1);")
          .expectError(
            '(SELECT 1)',
            type: AnalysisErrorType.cteColumnCountMismatch,
          );
      engine
          .analyze("UPDATE foo SET (a) = (SELECT 1,2);")
          .expectError(
            '(SELECT 1,2)',
            type: AnalysisErrorType.cteColumnCountMismatch,
          );
      engine
          ...
          .expectError(
            '(SELECT b FROM foo as f WHERE f.a=a)',
            type: AnalysisErrorType.cteColumnCountMismatch,
          );
    });
