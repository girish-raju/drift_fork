...
// long
const absentContent = TodosTableCompanion(
  title: Value('Test'),
  content: Value.absent(),
);
const shortTitle = TodosTableCompanion(
  title: Value('A'),
  content: Value('content'),
);
final TodosTableCompanion longTitle = TodosTableCompanion(
  title: Value('A ${'very' * 5} long title'),
  content: const Value('hi'),
);
const valid = TodosTableCompanion(
  title: Value('Test'),
...
void main() {
  ...
  test('detects errors on insert', () {
    expect(
      ...
      throwsA(
        predicate<InvalidDataException>(
