...
void main() {
  ...
  test('generates insert statements', () async {
    ...
    verify(
      executor.runInsert('INSERT INTO "todos" ("content") VALUES (?)', [
  ...
  test('generates insert or replace statements', () async {
    await db
        .into(db.todosTable)
        .insert(
          const TodoEntry(id: RowId(113), content: 'Done'),
  ...
  test('escaped when column name is keyword', () async {
    ...
    verify(
      executor.runInsert('INSERT INTO "pure_defaults" ("insert") VALUES (?)', [
        'foo',
      ]),
    );
  });
  ...
  group('on table instances', () {
    test('insert', () async {
      await db.categories.insert().insert(
        CategoriesCompanion.insert(description: 'description'),
      );

      verify(
        executor.runInsert('INSERT INTO "categories" ("desc") VALUES (?)', [
          'description',
        ]),
      );
    });
